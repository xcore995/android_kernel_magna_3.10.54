		    ...
		    I32S _optionSize)
{
	...
	if (Tcc353xApiControlAddress[_moduleIndex] != 0) {
		TcpalPrintErr((I08S *)
			      "[dtv][tcc3536][error][TCC353X] Tcc353xApiOpen - already opend\n");
		return TCC353X_RETURN_FAIL;
	}

	if (_optionSize > TCC353X_DIVERSITY_MAX * sizeof(Tcc353xOption_t)) {
		TcpalPrintErr((I08S *)
			      "[dtv][tcc3536][error][TCC353X] Tcc353xApiOpen - Too much optionsize\n");
		return TCC353X_RETURN_FAIL;
	} else {
		...
		if (needNumberofOptions != _optionSize) {
			TcpalPrintErr((I08S *)
				      "[dtv][tcc3536][error][TCC353X] Tcc353xApiOpen - optionsize mismatch\n");
		} else {
			...
			TcpalPrintLog((I08S *)
				      "[dtv][tcc3536][debug_info][TCC353X] Driver Core Version [%d. %d. %d]\n",
		    ...
		    I32S _codeSize, Tcc353xStreamFormat_t * _streamFormat)
{
	...
	if (Tcc353xApiControl[_moduleIndex][0].status ==
		...
		TcpalPrintErr((I08S *) "[dtv][tcc3536][exception][TCC353X] already initialized\n");
		return TCC353X_RETURN_FAIL;
	}
			...
			Tcc353xTuneOptions * _tuneOption, I32S _fastScan)
{
	...
	for (i = 0; i < tickCount; i++) {
		...
		if(userStopCmd[_moduleIndex] == 1) {
			...
			TcpalPrintLog ((I08S *)"[dtv][tcc3536][exception][TCC353X] user stop cmd! quit for loop!\n");
			return TCC353X_RETURN_FAIL;
		}
