...
inline static int moveDW9714AF(unsigned long a_u4Position)
{
	...
	//                                                                     
			...
			{
				breakDownSteps(10, 10, g_u4CurrPosition, (g_u4TargetPosition));
			}
			else if( (g_i4Dir == -1) && (g_u4CurrPosition!=g_u4TargetPosition) && (g_u4TargetPosition<500)){
...
static int DW9714AF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	...
	DW9714AFDB("[KYUNGHUN] DW9714AF_Release - Start[%d]\n",g_s4DW9714AF_RELEASE);
	...
	if (g_s4DW9714AF_Opened) {
        ...
        msleep(1);
        moveDW9714AF( (g_u4DW9714AF_INF+5) );
