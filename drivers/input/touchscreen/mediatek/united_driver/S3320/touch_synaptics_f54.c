...
static error_type ReadChCount(struct synaptics_ts_data *ts)
{
	...
	touch_i2c_read_byte(ts->client, ts->sensor_fc.dsc.data_base+1, rx_max_cnt, data);
	...
	touch_i2c_read_byte(ts->client, ts->sensor_fc.dsc.data_base+2, tx_max_cnt, data);
...
static int ReadTRexShort(struct synaptics_ts_data *ts)
{
	...
	touch_i2c_read_byte(ts->client, REPORT_DATA_REG, MaxArrayLength, cap_data);
