			  ...
			  I32U _initialCount)
{
	...
	if(index<0) {
		TcpalPrintErr((I08S *)"[dtv][tcc3536][error] Mutex Get Fail index=%d\n", (int)index);
	...
	TcpalPrintLog((I08S *)"[dtv][tcc3536][ok] MutexC %s lock=%d\n", _name, (int)(lock));
...
I32S TcpalDeleteSemaphore(TcpalSemaphore_t * _semaphore)
{
	...
	if(index < 0) 
	{ 
		TcpalPrintErr((I08S *)"[dtv][tcc3536][error] Mutex Delete Fail index=%d\n", (int)index);
		return TCC353X_RETURN_FAIL;
	}
	TcpalPrintLog((I08S *)"[dtv][tcc3536][ok] MutexR lock=%d\n", (int)(lock));
	mutex_destroy(lock);
	*_semaphore = 0;
	return TCC353X_RETURN_SUCCESS;
}
...
I32S TcpalSemaphoreLock(TcpalSemaphore_t * _semaphore)
{
	...
	if(lock == NULL) {
		TcpalPrintErr((I08S *)"[dtv][tcc3536][error] semaphore lock error\n");
		return TCC353X_RETURN_FAIL;
	}
	mutex_lock(lock);
	return TCC353X_RETURN_SUCCESS;
...
I32S TcpalSemaphoreUnLock(TcpalSemaphore_t * _semaphore)
{
	...
	if(lock == NULL) {
		TcpalPrintErr((I08S *)"[dtv][tcc3536][error] semaphore unlock error\n");
		return TCC353X_RETURN_FAIL;
	}
