...
#define SCAN_CHK_PERIOD 10 /* 1 ms */
...
s32 fc8300_init(HANDLE handle, DEVICEID devid)
{
    ...
    bbm_byte_write(handle, DIV_BROADCAST, BBM_HANGING_TS, 0xff);
    bbm_byte_write(handle, DIV_BROADCAST, BBM_HANGING_ENABLE, 0x01);

    bbm_byte_write(handle, DIV_MASTER, BBM_INT_AUTO_CLEAR, 0x01);
    #ifdef BBM_STATUS_AUTO_CLEAR
    bbm_byte_write(handle, DIV_MASTER, BBM_STATUS_AUTO_CLEAR_EN, 0x01);
    #endif

    bbm_byte_write(handle, DIV_MASTER, BBM_INT_PERIOD, 0xDE);
...
s32 fc8300_scan_status(HANDLE handle, DEVICEID devid)
{
    ...
    for (i = 0; i < ifagc_timeout; i++) {
        bbm_byte_read(handle, DIV_MASTER, 0x3025, &a);
        //print_log(0, "[FC8300] scan Status ifagc_timeout %d, %d\n", a, i);
    ...
    for (; i < ofdm_timeout; i++) {
        bbm_byte_read(handle, DIV_MASTER, 0x3025, &a);
        //print_log(0, "[FC8300] scan Status ofdm_timeout %d, %d\n", a, i);
    ...
    for (; i < ffs_lock_timeout; i++) {
        bbm_byte_read(handle, DIV_MASTER, 0x3026, &a);
        //print_log(0, "[FC8300] scan Status ffs_lock_timeout %d, %d\n", a, i);
    ...
    for (i = 0; i < cfs_timeout; i++) {
        bbm_byte_read(handle, DIV_MASTER, 0x3025, &a);
        //print_log(0, "[FC8300] scan Status cfs_timeout %d, %d\n", a, i);
    ...
    for (i = 0; i < tmcc_timeout; i++) {
        bbm_byte_read(handle, DIV_MASTER, 0x3026, &a);
        //print_log(0, "[FC8300] scan Status tmcc_timeout %d, %d\n", a, i);
    ...
    for (i = 0; i < ts_err_free_timeout; i++) {
        bbm_byte_read(handle, DIV_MASTER, 0x50c5, &a);
        //print_log(0, "[FC8300] scan Status ts_err_free_timeout %d, %d\n", a, i);
