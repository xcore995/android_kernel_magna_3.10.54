...
#define KNOCKCODE_DELAY			20  //150ms
...
static error_type synaptics_ts_page_data_write(struct i2c_client *client, u8 page, u8 reg, int size, u8 *data)
{
	DO_SAFE(touch_i2c_write_byte(client, PAGE_SELECT_REG, page), error);
...
static error_type synaptics_ts_page_data_write_byte(struct i2c_client *client, u8 page, u8 reg, u8 data)
{
	DO_SAFE(touch_i2c_write_byte(client, PAGE_SELECT_REG, page), error);
...
static error_type read_page_description_table(struct i2c_client *client)
{
	...
	for (page_num = 0; page_num < PAGE_MAX_NUM; page_num++)
	{
		...
		for (address = DESCRIPTION_TABLE_START; address > 10; address -= sizeof(struct function_descriptor))
		{
			DO_SAFE(touch_i2c_read(client, address, sizeof(buffer),	(unsigned char *)&buffer) < 0, error);
...
static error_type check_firmware_status(struct synaptics_ts_data *ts)
{
	...
	if ((device_status & DEVICE_STATUS_FLASH_PROG) || (device_status & DEVICE_CRC_ERROR_MASK) || (flash_status & 0xFF))
	{
...
static error_type tci_control(struct synaptics_ts_data *ts, int type, u8 value)
{
	...
	switch (type)
	{
		...
		case TCI_ENABLE_CTRL:
			...
			DO_SAFE(synaptics_ts_page_data_write(client, LPWG_PAGE, LPWG_TAPCOUNT_REG, 1, buffer), error);
			break;

		case TCI_ENABLE_CTRL2:
			DO_SAFE(synaptics_ts_page_data_read(client, LPWG_PAGE, LPWG_TAPCOUNT_REG2, 1, buffer), error);
			buffer[0] = (buffer[0] & 0xfe) | (value & 0x1);
			DO_SAFE(synaptics_ts_page_data_write(client, LPWG_PAGE, LPWG_TAPCOUNT_REG2, 1, buffer), error);
			break;
		...
		case MIN_INTERTAP_CTRL:
			DO_SAFE(synaptics_ts_page_data_write_byte(client, LPWG_PAGE, LPWG_MIN_INTERTAP_REG, value), error);
			break;

		case MIN_INTERTAP_CTRL2:
			DO_SAFE(synaptics_ts_page_data_write_byte(client, LPWG_PAGE, LPWG_MIN_INTERTAP_REG2, value), error);
			break;

		case MAX_INTERTAP_CTRL:
			DO_SAFE(synaptics_ts_page_data_write_byte(client, LPWG_PAGE, LPWG_MAX_INTERTAP_REG, value), error);
			break;

		case MAX_INTERTAP_CTRL2:
			DO_SAFE(synaptics_ts_page_data_write_byte(client, LPWG_PAGE, LPWG_MAX_INTERTAP_REG2, value), error);
			break;

		case TOUCH_SLOP_CTRL:
			DO_SAFE(synaptics_ts_page_data_write_byte(client, LPWG_PAGE, LPWG_TOUCH_SLOP_REG, value), error);
			break;

		case TOUCH_SLOP_CTRL2:
			DO_SAFE(synaptics_ts_page_data_write_byte(client, LPWG_PAGE, LPWG_TOUCH_SLOP_REG2, value), error);
			break;

		case TAP_DISTANCE_CTRL:
			DO_SAFE(synaptics_ts_page_data_write_byte(client, LPWG_PAGE, LPWG_TAP_DISTANCE_REG, value), error);
			break;

		case TAP_DISTANCE_CTRL2:
			DO_SAFE(synaptics_ts_page_data_write_byte(client, LPWG_PAGE, LPWG_TAP_DISTANCE_REG2, value), error);
			break;

		case INTERRUPT_DELAY_CTRL:
			buffer[0] = value ? ( (KNOCKON_DELAY << 1) | 0x1 ) : 0;
			DO_SAFE(synaptics_ts_page_data_write_byte(client, LPWG_PAGE, LPWG_INTERRUPT_DELAY_REG, buffer[0]), error);
			break;

		case INTERRUPT_DELAY_CTRL2:
			buffer[0] = value ? ( (KNOCKCODE_DELAY << 1) | 0x1 ) : 0;
			DO_SAFE(synaptics_ts_page_data_write_byte(client, LPWG_PAGE, LPWG_INTERRUPT_DELAY_REG2, buffer[0]), error);
			break;
...
static error_type get_lpwg_data(struct synaptics_ts_data *ts, int count)
{
	...
	DO_SAFE(i = synaptics_ts_page_data_read(client, LPWG_PAGE, LPWG_DATA_REG, 4*count, &buffer[0][0]), error);

	for (i = 0; i < count; i++)
	{
		ts->pw_data.data[i].x = GET_X_POSITION(buffer[i][1], buffer[i][0]);
		ts->pw_data.data[i].y = GET_Y_POSITION(buffer[i][3], buffer[i][2]);
...
static error_type lpwg_control(struct synaptics_ts_data *ts)
{
	...
	switch (ts->reportMode)
	{
		...
		case T_REPORT_KNOCK_ON_CODE:					// TCI-1 and TCI-2
			...
			tci_control(ts, INTERRUPT_DELAY_CTRL2, 1);	// interrupt delay = 0ms
...
error_type synaptics_ts_init(struct i2c_client *client)
{
	...
	DO_SAFE(synaptics_ts_page_data_write_byte(client, ANALOG_PAGE, DYNAMIC_SENSING_CONTROL_REG, SENSING_CONTROL_120HZ), error);

	TouchReset();

	DO_SAFE(synaptics_ts_page_data_write_byte(client, ANALOG_PAGE, DYNAMIC_SENSING_CONTROL_REG, SENSING_CONTROL_AUTO), error);

	DO_SAFE(touch_i2c_write_byte(client, DEVICE_CONTROL_REG, DEVICE_CONTROL_NOSLEEP | DEVICE_CONTROL_CONFIGURED), error);
...
error_type synaptics_ts_isr(struct i2c_client *client, TouchReadData *pData)
{
	...
	{
		touch_i2c_read(client, FINGER_DATA_REG_START, (NUM_OF_EACH_FINGER_DATA_REG * MAX_NUM_OF_FINGERS), (u8 *)regFingerData);
	...
	else if ( regIntStatus & INTERRUPT_MASK_CUSTOM )
	{
		...
		{
			LGTC_LOG( "OverTap Knock-code Detected\n" );
			pData->type = T_DATA_KNOCK_CODE;
			synaptics_ts_page_data_read(client, LPWG_PAGE, LPWG_OVER_TAPCOUNT, 1, &tap_count);
			LGTC_LOG( "Tap Count: %d\n", tap_count );
...
error_type synaptics_ts_ic_ctrl(struct i2c_client *client, u8 code, u32 value, u32 *ret)
{
	u8 buf = 0;
	...
	switch (code)
	{
		case IC_CTRL_READ:
			...
			*ret = (u32)buf;
			break;
		case IC_CTRL_WRITE:
			DO_SAFE(touch_i2c_write_byte(client, ((value & 0xFF00) >> 8), (value & 0xFF)), error);
			break;
		default:
			break;
	}
...
error_type synaptics_f54_test(struct i2c_client *client, char* buf, int* raw_status, int* ch_status)
{
	struct synaptics_ts_data *ts = (struct synaptics_ts_data *)get_touch_handle(client);
