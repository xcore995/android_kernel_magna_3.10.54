...
#define _DO_NOT_USE_MUTEX_ /* special case for speed up irq reaction */

#if !defined (_DO_NOT_USE_MUTEX_)
...
#define RING_DATA_BUFFER_SIZE 188*300*20
...
void mbt_dataring_create(unsigned int *buffer_id, int len)
{
	...
	//if(mutex)
	{
		...
		if(rbuf == NULL)
		{
			...
			printk("[dtv][tcc3536][error] mbt_dataring_create fail no memory !!! \n");
			return;
		}
...
int mbt_dataring_read(unsigned int buffer_id, char* buf, int len)
{
	...
	//-------- Bug fix 2014 08 05 --------------------
	len = mbt_ring_read(rbuf, (char*)buf, (int)len);
