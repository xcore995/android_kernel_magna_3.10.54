...
int ReadReport(unsigned char input, char *buf)
{
	...
	switch (input){
		...
		case 'c':
			...
			break;
		case 'd':
			//			fLGTC_DBG(stderr, "Press any key to continue after you have lowered the bar.\n");
			//			_getch();
			ReadTRexOpenReport();
			break;
		case 'e':
...
bool TestPreparation(void)
{
	...
	if(count >= DefaultTimeout) {
		...
		//LGTC_DBG("Timeout -- ForceUpdate can not complete\n");
		Reset();
		return false;
	}
	...
	if(count >= DefaultTimeout){
		...
		//LGTC_DBG("Timeout -- ForceCal can not complete\n");
		Reset();
		return false;
	}
