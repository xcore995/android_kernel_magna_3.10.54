			      ...
			      _pidTableControl)
{
	...
	if (h->numOfPidTable + _pidTableControl->numberOfPid > 
	    MAX_PID_FILTER_CNT) {
		TcpalPrintErr((I08S *)"[dtv][tcc3536][error][TCC353X] MAX PID count over\n");
		return TCC353X_RETURN_FAIL;
	}
	...
	for (i=0; i<_pidTableControl->numberOfPid; i++)	{
		...
		if(slot == -1)	{
			TcpalPrintLog((I08S *)"[dtv][tcc3536][error][TCC353X] PID=[0x%x] Exist\n",
			...
			TcpalPrintLog((I08S *)"[dtv][tcc3536][ok][TCC353X] Add PID=[0x%x]\n",
				_pidTableControl->Pid[i]);
				 ...
				 _pidTableControl)
{
	...
	if (h->numOfPidTable < _pidTableControl->numberOfPid) {
		TcpalPrintErr((I08S *)"[dtv][tcc3536][error][TCC353X] PID count underrun\n");
		return TCC353X_RETURN_FAIL;
	}
	...
	for (i=0; i<_pidTableControl->numberOfPid; i++)	{
		...
		if(slot == -1)	{
			TcpalPrintLog((I08S *)"[dtv][tcc3536][error][TCC353X] PID=[0x%x] None\n",
				_pidTableControl->Pid[i]);
		} else if (slot>31) {
			TcpalPrintLog((I08S *)"[dtv][tcc3536][debug_info][TCC353X] PID=[0x%x] None\n",
				_pidTableControl->Pid[i]);
		} else {
			h->pidTable[slot]= 0x00;
			h->numOfPidTable--;

			TcpalPrintLog((I08S *)"[dtv][tcc3536][ok][TCC353X] Remove PID=[0x%x]\n",
				_pidTableControl->Pid[i]);
			/* apply pid table to register */
			idx = ((I32U)(slot)>>1);
