...
#include "../inc/tcc353x_mailbox.h"
#include "../inc/tcc353x_core.h"
#include "../inc/tcc353x_register_control.h"
#include "../inc/tcc353x_rf.h"
#include "../inc/tcc353x_isdb.h"
#include "../inc/tcc353x_command_control.h"
#include "../inc/tcc353x_dpll_38400osc.h"
#include "../inc/tcc353x_dpll_26000osc.h"
#include "../inc/tcc353x_dpll_19200osc.h"
...
I32U Tcc353xCoreVersion = ((0<<16) | (1<<8) | (95));
I32U Tcc3531_dpll_table_version = ((0<<24) | (5<<16) | (0<<8) | (0));
I32U Tcc3535_dpll_table_version = ((0<<24) | (12<<16) | (0<<8) | (0));
I32U Tcc3536_dpll_table_version = ((0<<24) | (12<<16) | (0<<8) | (0));
...
extern I32U OSC_26000_DpllTable_Partial1Seg_tcc3536[];
extern I32U OSC26000_DpllTable_FullSeg_tcc3536[];
...
I32S Tcc353xCheckTcc3531Version(I32S _moduleIndex)
{
	...
	if((mailbox.data_array[0]>>31) & 0x01) {
		if(Tcc353xHandle[_moduleIndex][0].options.
			...
			ver = VER_TCC3531_2AX;
			TcpalPrintLog((I08S *)
				"[dtv][tcc3536][ok][TCC353X] TCC3531 Chip is [2AX] version\n");
		} else if(Tcc353xHandle[_moduleIndex][0].options.
		    basebandName==BB_TCC3532) {
			ver = VER_TCC3532_0AX;
			TcpalPrintLog((I08S *)
				"[dtv][tcc3536][ok][TCC353X] TCC3532 Chip is [0AX] version\n");
		} else {
			ver = VER_TCC3531_2AX;
			TcpalPrintLog((I08S *)
				"[dtv][tcc3536][ok][TCC353X] TCC3531 Chip is [2AX] version\n");
		...
		if(Tcc353xHandle[_moduleIndex][0].options.
			...
			ver = VER_TCC3531_1XX;
			TcpalPrintLog((I08S *)
				"[dtv][tcc3536][ok][TCC353X] TCC3531 Chip is [1XX] version\n");
		} else if(Tcc353xHandle[_moduleIndex][0].options.
		    basebandName==BB_TCC3532) {
			ver = VER_TCC3532_0XX;
			TcpalPrintLog((I08S *)
				"[dtv][tcc3536][ok][TCC353X] TCC3532 Chip is [0XX] version\n");
		} else {
			ver = VER_TCC3531_1XX;
			TcpalPrintLog((I08S *)
				"[dtv][tcc3536][ok][TCC353X] TCC3531 Chip is [1XX] version\n");
...
I32S Tcc353xInit(I32S _moduleIndex, I08U * _coldbootData, I32S _codeSize)
{
	...
	mailbox_t MailBox;
	I08U progId;
	I08U version0, version1, version2, release_version_flag;
	...
	for (i = 0; i < Tcc353xCurrentDiversityCount[_moduleIndex]; i++) {
		...
		*/
		...
		TcpalPrintLog((I08S *)
			      "[dtv][tcc3536][debug_info][TCC353X] ----------------------\n");
		TcpalPrintLog((I08S *)
			"[dtv][tcc3536][debug_info][TCC353X] Code VersionForm=%d addr=0x%08X\n", i,
			temp);
		...
		TcpalPrintLog((I08S *)
			      "[dtv][tcc3536][debug_info][TCC353X] Version=(%d.%d.%d)\n", version0, 
			      version1, version2);
		TcpalPrintLog((I08S *)
			      "[dtv][tcc3536][debug_info][TCC353X] Date=(%d.%d.%d)\n", year, 
			      month, date);
		...
		TcpalPrintLog((I08S *)
			      "[dtv][tcc3536][debug_info][TCC353X] Device Driver Version=(%d.%d.%d)\n", 
	...
	case BB_TCC3531:
	case BB_TCC3532:
		version0 = (I08U)((Tcc3531_dpll_table_version>>24)&0xFF);
		version1 = (I08U)((Tcc3531_dpll_table_version>>16)&0xFF);
		version2 = (I08U)((Tcc3531_dpll_table_version>>8)&0xFF);
		release_version_flag = 
			(I08U)((Tcc3531_dpll_table_version)&0xFF);

		TcpalPrintLog((I08S *)
			      "[dtv][tcc3536][debug_info][TCC353X] DPLL Version=(%d.%d.%d)-[%d]\n", 
			      version0, version1, version2,
			      release_version_flag);
		break;
	case BB_TCC3535:
		version0 = (I08U)((Tcc3535_dpll_table_version>>24)&0xFF);
		version1 = (I08U)((Tcc3535_dpll_table_version>>16)&0xFF);
		version2 = (I08U)((Tcc3535_dpll_table_version>>8)&0xFF);
		release_version_flag = 
			(I08U)((Tcc3535_dpll_table_version)&0xFF);
		...
		break;
	case BB_TCC3536:
		version0 = (I08U)((Tcc3536_dpll_table_version>>24)&0xFF);
		version1 = (I08U)((Tcc3536_dpll_table_version>>16)&0xFF);
		version2 = (I08U)((Tcc3536_dpll_table_version>>8)&0xFF);
		release_version_flag = 
			(I08U)((Tcc3536_dpll_table_version)&0xFF);
	...
	/* optimize for cspi only mode - stream read address fix*/
	{
	I32U address;
	I08U inputValue[4];
	...
	Tcc353xSetRegDmaSourceAddress(&Tcc353xHandle[0][0], &inputValue[0], _UNLOCK_);
	}
...
static I16U Tcc353xGetNewPLL_26000(I32S _moduleIndex, 
    Tcc353xTuneOptions * _tuneOption, I32S _Inputfrequency)
{
	I16U newPll = 0;
	
#if defined (_SUPPORT_OSC_26000_)
	switch (_tuneOption->segmentType) {
	case TCC353X_ISDBT_1_OF_13SEG:
		newPll = (I16U)(Tcc353xSearchDpllValue (
				(I32U)(_Inputfrequency),
				&OSC_26000_DpllTable_Partial1Seg_tcc3536[0],
				_OSC_26000_MAX_1SEG_FREQ_NUM_, 
				OSC_260_PLL_ISDB_T_1SEG_2F));
		break;
	case TCC353X_ISDBT_13SEG:
		newPll = (I16U)(Tcc353xSearchDpllValue (
				(I32U)(_Inputfrequency),
				&OSC26000_DpllTable_FullSeg_tcc3536[0],
				_OSC_26000_MAX_13SEG_FREQ_NUM_, 
				OSC_260_PLL_ISDB_T_13SEG_3F));
		break;
	case TCC353X_ISDBTMM: /* TBD */
		if (_tuneOption->tmmSet == UserDefine_Tmm13Seg)
			newPll = (I16U)(Tcc353xSearchDpllValue (
					(I32U)(_Inputfrequency),
			...
			newPll = (I16U)(Tcc353xSearchDpllValue (
					...
					PLL_ISDB_TMM_PARTIAL_1_SEG));
		break;
	case TCC353X_ISDBTSB_1SEG: /* TBD */
	case TCC353X_ISDBTSB_1_OF_3SEG: /* TBD */
		newPll = (I16U)(Tcc353xSearchDpllValue (
					(I32U)(_Inputfrequency),
	...
	case TCC353X_ISDBTSB_3SEG: /* TBD */
		...
		break;
	default:
		newPll = OSC_260_PLL_ISDB_T_13SEG_3F;
		break;
	}
#endif
	return newPll;
		 ...
		 Tcc353xTuneOptions * _tuneOption, I32S _fastTune)
{
	...
	basebandName = Tcc353xHandle[_moduleIndex][0].options.basebandName;
	...
	if(Tcc353xHandle[_moduleIndex][0].options.streamInterface== 
		...
		case TCC353X_ISDBTSB_1SEG:
		case TCC353X_ISDBTSB_1_OF_3SEG:
			if(basebandName==BB_TCC3530)
				bufferEndAddress = 0x00019F5B;
			else
				bufferEndAddress = 0x00019FFB;
			break;
		case TCC353X_ISDBT_13SEG:
		case TCC353X_ISDBTSB_3SEG:
			if(basebandName==BB_TCC3530)
		...
		/* change TCC353X_BUFF_A_END for segment
		Register 
		- bufferConfig_0x52
		- bufferConfig_0x53
		*/

		if(basebandName==BB_TCC3530 || basebandName==BB_TCC3536) {
			bufferEndReg[0] = 
				(I08U)((bufferEndAddress >> 10) & 0xFF),
			bufferEndReg[1] = 
				(I08U)((bufferEndAddress >> 2) & 0xFF),
	...
	case TCC353X_ISDBT_13SEG:
		/* ISDB-T */
		TcpalPrintLog((I08S *)
			      "[dtv][tcc3536][debug_info][TCC353X] [BB=%d] Tune frequency=%d [ISDB-T]\n",
	...
	case TCC353X_ISDBTSB_1_OF_3SEG:
		/* ISDB-Tsb */
		TcpalPrintLog((I08S *)
			      "[TCC353X] [BB=%d] Tune frequency=%d [ISDB-Tsb]\n",
			      _moduleIndex, _frequency);
		break;
	...
	case TCC353X_ISDBTMM:
		...
		TcpalPrintLog((I08S *)
 			      "[dtv][tcc3536][debug_info][TCC353X] [BB=%d] Tune frequency [ISDB-TMM] frequency=%d, Inputfrequency=%d\n",
			      _moduleIndex, _frequency, Inputfrequency);
		break;
	default:
		/* ISDB-T */
		TcpalPrintLog((I08S *)
			      "[dtv][tcc3536][debug_info][TCC353X] [BB %d] Tune frequency [ISDB-T]=%d\n",
			      _moduleIndex, _frequency);
		break;
	}
	...
	{
		if(basebandName == BB_TCC3535)
			newPll = Tcc353xGetNewPLL_tcc3535
				    (_moduleIndex, _tuneOption, Inputfrequency);
		else if(basebandName == BB_TCC3536 && 
		   Tcc353xHandle[_moduleIndex][0].options.oscKhz == 26000)
			newPll = Tcc353xGetNewPLL_26000
				    (_moduleIndex, _tuneOption, Inputfrequency);
		else if(basebandName == BB_TCC3536)
	...
	TotalTime = (I32U)((I64U)(TcpalGetTimeIntervalCount_ms(CurrTime)) 
				   & 0xFFFFFFFF);
	TcpalPrintStatus ((I08S *) "[dtv][tcc3536][debug_info][TCC353X] SpendTime=%dms\n", TotalTime);
	return TCC353X_RETURN_SUCCESS;
}
...
I32U Tcc353xSendStartMail(Tcc353xHandle_t * _handle)
{
	...
	if (retmail == -1)
		TcpalPrintErr((I08S *)
			      "[dtv][tcc3536][error][TCC353X] Error - Sending Startmail\n");
	return retmail;
}
			  ...
			  Tcc353xOption_t * _Tcc353xOption)
{
	...
	case TCC353X_BOARD_SINGLE:
		...
		TcpalPrintLog((I08S *)
			      "[dtv][tcc3536][ok][TCC353X] TCC353X Attach Success! [Single Mode]\n");
		break;

	case TCC353X_BOARD_2DIVERSITY:
		Tcc353xCurrentDiversityCount[_moduleIndex] = 2;
		TcpalPrintLog((I08S *)
			      "[dtv][tcc3536][ok][TCC353X] TCC353X Attach Success! [2Diversity Mode]\n");
		break;

	case TCC353X_BOARD_3DIVERSITY:
		Tcc353xCurrentDiversityCount[_moduleIndex] = 3;
		TcpalPrintLog((I08S *)
 			      "[dtv][tcc3536][ok][TCC353X] TCC353X Attach Success! [3Diversity Mode]\n");
		break;

	case TCC353X_BOARD_4DIVERSITY:
		Tcc353xCurrentDiversityCount[_moduleIndex] = 4;
		TcpalPrintLog((I08S *)
			      "[dtv][tcc3536][ok][TCC353X] TCC353X Attach Success! [4Diversity Mode]\n");
		break;
	...
	if (pTcc353xInterfaceSema == NULL) {
		...
		TcpalPrintLog((I08S *)
			      "[dtv][tcc3536][exception][TCC353X] - Already exist interface semaphore\n");
	}
	...
	for (i = 0; i < Tcc353xCurrentDiversityCount[_moduleIndex]; i++) {
		...
		Tcc353xGetRegChipId(&Tcc353xHandle[_moduleIndex][i],
				    &chipId);
		TcpalPrintLog((I08S *) "[dtv][tcc3536][debug_info][TCC353X][%d][%d] ChipID=0x%02x\n",
			      _moduleIndex, i, chipId);
		Tcc353xGetRegProgramId(&Tcc353xHandle[_moduleIndex][i],
				       &progId);
		TcpalPrintLog((I08S *) "[dtv][tcc3536][debug_info][TCC353X][%d][%d] progId=0x%02x\n",
			      _moduleIndex, i, progId);
	}

	switch(Tcc353xHandle[_moduleIndex][0].options.basebandName) {
	case BB_TCC3530:
		TcpalPrintLog((I08S *) "[dtv][tcc3536][ok][TCC353X] TCC3530 selected\n");
		break;
	case BB_TCC3531:
		TcpalPrintLog((I08S *) "[dtv][tcc3536][ok][TCC353X] TCC3531 selected\n");
		break;
	case BB_TCC3532:
		TcpalPrintLog((I08S *) "[dtv][tcc3536][ok][TCC353X] TCC3532 selected\n");
		break;
	case BB_TCC3535:
		TcpalPrintLog((I08S *) "[dtv][tcc3536][ok][TCC353X] TCC3535 selected\n");
		break;
	case BB_TCC3536:
		TcpalPrintLog((I08S *) "[dtv][tcc3536][ok][TCC353X] TCC3536 selected\n");
		break;
	default:
		TcpalPrintErr((I08S *) "[dtv][tcc3536][ok][TCC353X] No baseband name selected\n");
		break;
	}
	...
	for (i = 0; i < Tcc353xCurrentDiversityCount[_moduleIndex]; i++) {
		...
		if(h->options.rfType == TCC353X_TRIPLE_BAND_RF) {
		   	if(h->options.basebandName != BB_TCC3531 && 
				...
				TcpalPrintLog((I08S *) "[dtv][tcc3536][ok][TCC353X] [%dth] Change to Dual band\n",i);
			} else {
				TcpalPrintLog((I08S *) "[dtv][tcc3536][ok][TCC353X] [%dth] Triple Band selected\n",i);
			}
		} else {
			if(h->options.basebandName == BB_TCC3535 ||
			   h->options.basebandName == BB_TCC3536) {
		   	   	h->options.rfType = TCC353X_TRIPLE_BAND_RF;
				TcpalPrintLog((I08S *) "[dtv][tcc3536][ok][TCC353X] [%dth] Change to Triple band\n",i);
			} else {
				TcpalPrintLog((I08S *) "[dtv][tcc3536][ok][TCC353X] [%dth] Dual Band selected\n",i);
			}
		}
	}
...
I32S Tcc353xDetach(I32S _moduleIndex)
{
	...
	TcpalPrintLog((I08S *) "[dtv][tcc3536][ok][TCC353X] Detach Success!\n");
				...
				Tcc353xBoot_t * _boot)
{
	...
	for (i = count - 1; i >= 0; i--) {
		...
		if (destCrc == srcCrc) {
			TcpalPrintLog((I08S *)
				      "[dtv][tcc3536][ok][TCC353X] [%d][%d] CRC Success!\n",
				      handle->moduleIndex,
				      handle->diversityIndex);
			TcpalPrintLog((I08S *)
				      "[dtv][tcc3536][debug_info][TCC353X] [0x%x][0x%x]\n",
				      srcCrc, destCrc);
		} else {
			TcpalPrintErr((I08S *)
				      "[dtv][tcc3536][error][TCC353X] [%d][%d] CRC Fail!!!!!! \n",
				      _moduleIndex, i);
			TcpalPrintErr((I08S *)
				      "[dtv][tcc3536][error][TCC353X] [0x%x][0x%x]\n",
				      srcCrc, destCrc);
			ret = TCC353X_RETURN_FAIL;
		}
	}
				    ...
				    I08U * _coldbootData, I32S _codeSize)
{
	...
	else if (Tcc353xHandle[_moduleIndex][0].options.oscKhz == 26000)
		plls = OSC_260_PLL_ISDB_T_13SEG_3F;
	else
		plls = PLL_ISDB_T_FULLSEG;
...
static void Tcc353xSetStreamControl(Tcc353xHandle_t * _handle)
{
	...
	if ((_handle->options.Config->periConfig_0x30 & 0x30) == 0x10) {
		...
		TcpalPrintLog((I08S *)
			      "[dtv][tcc3536][debug_info][TCC353X] SET SPI Clk=%d khz [DLR=%d]\n",
			      streamClkSpeed, dlr);
		...
		streamClkSpeed = (I32U) (temp);
		TcpalPrintLog((I08S *)
			      "[dtv][tcc3536][debug_info][TCC353X] SET TS Clk=%d khz [DLR=%d]\n",
			      streamClkSpeed, dlr);
	} else {
		;		/*none */
	}
			  ...
			  I32S _flagInterfaceLock, I16U _pllValue)
{
	...
	h->mainClkKhz = fout;
	TcpalPrintLog((I08S *)"[dtv][tcc3536][debug_info][TCC353X] PLLSet=%dkHz\n", h->mainClkKhz);
	return TCC353X_RETURN_SUCCESS;
}
			      ...
			      Tcc353xOption_t * _Tcc353xOption)
{
	...
	default:
		...
		Tcc353xHandle[_moduleIndex][_diversityIndex].originalAddress =
		    ...
		    options.address;
		TcpalPrintErr((I08S *)
			      "[dtv][tcc3536][error][TCC353X] Driver Can't support your interface yet\n");
		break;
	}
}
					   ...
					   I08S _commandInterface)
{
	...
	case TCC353X_IF_I2C:
		...
		Tcc353xHandle[_moduleIndex][i].originalAddress =
		    Tcc353xHandle[_moduleIndex][i].options.address;
		TcpalPrintLog((I08S *)
			      "[dtv][tcc3536][ok][TCC353X] Interface is I2C\n");
		break;
	...
	case TCC353X_IF_TCCSPI:
		...
		Tcc353xHandle[_moduleIndex][i].originalAddress =
		    (Tcc353xHandle[_moduleIndex][i].
		     options.address >> 1);
		TcpalPrintLog((I08S *)
			      "[dtv][tcc3536][ok][TCC353X] Interface is Tccspi\n");
		break;
				      ...
				      Tcc353xBoot_t * pBOOTBin)
{
	...
	I08U *dintDataPtr;
	I08U *randDataPtr;
	/*I08U *colOrderDataPtr;*/	/* for cpp check */
	...
	if (size >= 8) {
		...
		/*size -= (length + 8);*/	/* cpp check warning */
	}
...
static I32S Tcc353xApplySpurSuppression_26000 (I32S _moduleIndex, 
					 Tcc353xTuneOptions *_tuneOption, 
					 I64U *_rcStep, I32U *_adcClkCfg,
					 I32U *_icic, I32U _frequencyInfo)
{
				     ...
				     I32U * _opConfig, I32U _frequencyInfo)
{
	...
	if(Tcc353xHandle[_moduleIndex][0].options.streamInterface== 
		...
		if(Tcc353xHandle[_moduleIndex][0].options.basebandName == 
			...
			if(S_TYPE == 0)
				...
				_tuneOption->segmentType ==TCC353X_ISDBTMM)
				OM_MODE = 0x1c;
			else if(S_TYPE ==2 && 
				_tuneOption->segmentType == TCC353X_ISDBT_13SEG)
				OM_MODE = 0x1c;
			else if(S_TYPE ==1 && 
				_tuneOption->segmentType ==TCC353X_ISDBTSB_3SEG)
				OM_MODE = 0x1c;
			else
				OM_MODE = 0x00;
		} else {
			if(S_TYPE == 0)
				...
				_tuneOption->segmentType ==TCC353X_ISDBTMM)
				OM_MODE = 0x1C; /* TCC3536 */
			else if(S_TYPE ==2 && 
				_tuneOption->segmentType == TCC353X_ISDBT_13SEG)
				OM_MODE = 0x18; /* TCC3536 */
			else if(S_TYPE ==1 && 
				_tuneOption->segmentType ==TCC353X_ISDBTSB_3SEG)
				OM_MODE = 0x1C; /* TCC3536 */
			else
	...
	if(Tcc353xHandle[_moduleIndex][0].TuneOptions.BandwidthMHz == 8) {
		...
		if(Tcc353xHandle[_moduleIndex][0].options.basebandName 
		   ...
		   == BB_TCC3536 &&  
		   Tcc353xHandle[_moduleIndex][0].options.oscKhz == 26000)
			Tcc353xApplySpurSuppression_26000(_moduleIndex, 
						    _tuneOption, 
						    &RC_STEP, &ADC_CLK_CFG, 
						    &ICIC, _frequencyInfo);
		else if(Tcc353xHandle[_moduleIndex][0].options.basebandName 
	...
	_opConfig[2] =
	   (DIV_NUM_CFG<<31) | (PDC<<27) | (SSE<<26) | (AID<<25) |
	...
	case BB_TCC3536:
		...
		break;
	default:
		TcpalPrintErr((I08S *) "[dtv][tcc3536][error][TCC353X] No baseband name selected\n");
		break;
	}
}
...
static I32U Tcc353xDspRestart(I32S _moduleIndex, I32S _diversityIndex)
{
	...
	TcpalSemaphoreUnLock(&Tcc353xOpMailboxSema[_moduleIndex]
			     [_diversityIndex]);
	TcpalPrintLog((I08S *) "[dtv][tcc3536][ok][TCC353X] SYS_RESET(DSP)\n");
	return TCC353X_RETURN_SUCCESS;
}
				   ...
				   I32S _diversityUnlockEnforcement)
{
	...
	if(Tcc353xCurrentDiversityCount[_moduleIndex]<2) {
		TcpalPrintErr((I08S *)"[dtv][tcc3536][error][TCC353X] Tcc353xDiversityUnlockEnforce Parameter Error\n");
		return TCC353X_RETURN_FAIL;
	}
...
{
	TcpalPrintLog((I08S *)
		      "[dtv][tcc3536][debug_info][TCC353X] Access dummy function 0\n");
	return TCC353X_RETURN_SUCCESS;
}
...
{
	TcpalPrintLog((I08S *)
		      "[dtv][tcc3536][debug_info][TCC353X] Access dummy function 1\n");
	return TCC353X_RETURN_SUCCESS;
}
