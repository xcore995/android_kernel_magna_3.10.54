    ...
    )
{
	...
	if (u4PktLen < HIF_RX_HDR_SIZE)
	{
	printk("u4PktLen=%u, ucHerDerLenOffset=%d, u2SeqNoTid=%d", u4PktLen, prHifRxHdr->ucHerderLenOffset, prHifRxHdr->u2SeqNoTid);
	}
    ...
    do {
        ...
        for(rxNum = 0 ; rxNum < 2 ; rxNum++) {
            ...
            for (i = 0; i < u4RxAggCount; i++) {
				...
				if (prHifRxHdr->u2PacketLen < HIF_RX_HDR_SIZE)
				{
				printk("u2PktLength %d\n", u2PktLength);
				dumpMemory8(ANDROID_LOG_ERROR,(PUINT_8)prHifRxHdr, u2PktLength > 100 ? 100:u2PktLength);
				}
