...
static void get_onseg_sig_info(struct broadcast_dmb_control_info *pInfo)
{
	U32 bb_cnr;

	pInfo->sig_info.info.oneseg_info.lock = get_lock_status();

	pInfo->sig_info.info.oneseg_info.cn = get_sig_cnr(&bb_cnr);
	if (pInfo->sig_info.info.oneseg_info.cn >= 2800)
		pInfo->sig_info.info.oneseg_info.cn = 2800;

	pInfo->sig_info.info.oneseg_info.ber = rtvISDBT_GetBER();
	pInfo->sig_info.info.oneseg_info.per = get_sig_per();
	pInfo->sig_info.info.oneseg_info.agc = rtvISDBT_GetAGC();
	pInfo->sig_info.info.oneseg_info.rssi = get_sig_rssi();
	pInfo->sig_info.info.oneseg_info.ErrTSP = mtv222_cb->err_tsp_cnt;
	pInfo->sig_info.info.oneseg_info.TotalTSP = mtv222_cb->total_tsp_cnt;
	//pInfo->sig_info.info.oneseg_info.antenna_level = get_sig_antenna_level(bb_cnr);
    pInfo->sig_info.info.mmb_info.antenna_level_oneseg = get_sig_antenna_level(bb_cnr);
	...
	MTVMSG("lock(%d), bb_cnr(%u), cn(%d), ber(%d), per(%d), rssi(%d), ErrTSP(%d), TotalTSP(%d), antenna_level(%d)\n",
			pInfo->sig_info.info.oneseg_info.lock, bb_cnr, pInfo->sig_info.info.oneseg_info.cn, pInfo->sig_info.info.oneseg_info.ber, pInfo->sig_info.info.oneseg_info.per,
			pInfo->sig_info.info.oneseg_info.rssi, pInfo->sig_info.info.oneseg_info.ErrTSP, pInfo->sig_info.info.oneseg_info.TotalTSP, pInfo->sig_info.info.mmb_info.antenna_level_oneseg /*pInfo->sig_info.info.oneseg_info.antenna_level*/);

	SHOW_DEBUG_TSPB_STAT;
}

int	broadcast_mtv_drv_if_power_on(void)
{
	int rc = OK;
	printk(KERN_DEBUG"[1seg]broadcast_mtv_drv_if_power_on\n");
...
int	broadcast_mtv_drv_if_power_off(void)
{
	mutex_lock(&mtv222_cb->ioctl_lock);
...
int	broadcast_mtv_drv_if_open(void)
{
	int rc = OK;
...
int	broadcast_mtv_drv_if_close(void)
{
	mutex_lock(&mtv222_cb->ioctl_lock);
...
int	broadcast_mtv_drv_if_set_channel(struct broadcast_dmb_set_ch_info *udata)
{
	int demod_ret;
	int rc = OK;
	...
	if (udata->mode == 0x01)//1) /* Scan mode */
...
int	broadcast_mtv_drv_if_resync(void)
{
...
int	broadcast_mtv_drv_if_detect_sync(struct broadcast_dmb_sync_info *udata)
{
	mutex_lock(&mtv222_cb->ioctl_lock);
...
int	broadcast_mtv_drv_if_get_sig_info(struct broadcast_dmb_control_info *pInfo)
{
    U32 bb_cnr;
	...
	case 1: /* all */
		fullseg_info->cn = get_sig_cnr(&bb_cnr);
		fullseg_info->ber_a = rtvISDBT_GetBER();
		fullseg_info->per_a = get_sig_per();
		fullseg_info->layerinfo_a = get_sig_layer_info();
		fullseg_info->tmccinfo=0xFF;
		fullseg_info->receive_status=0xFF;
		fullseg_info->rssi= get_sig_rssi();
		fullseg_info->scan_status=0xFF;
		fullseg_info->sysinfo=0xFF;
		fullseg_info->total_tsp_a = mtv222_cb->total_tsp_cnt;
		...
		fullseg_info->antenna_level_oneseg = get_sig_antenna_level(bb_cnr);
		fullseg_info->agc = rtvISDBT_GetAGC();
		fullseg_info->ber_1seg = rtvISDBT_GetBER();
		fullseg_info->per_1seg = get_sig_per();
		fullseg_info->total_tsp_1seg = mtv222_cb->total_tsp_cnt;
		fullseg_info->err_tsp_1seg = mtv222_cb->err_tsp_cnt;
	...
	case 7: /* Receive status */
		fullseg_info->receive_status = get_sig_receive_status();
		break;
	...
	case 9: /* scan status */
		fullseg_info->scan_status = get_sig_scan_status();
		break;

	case 10: /* sys info */
		fullseg_info->sysinfo = get_sig_sys_info();
		break;

	case 11: /* tmcc info */
		fullseg_info->tmccinfo = get_sig_tmcc_info();
		break;

	case 12: /* oneseg_sig_info */
		get_onseg_sig_info(pInfo);
		break;
...
int	broadcast_mtv_drv_if_get_ch_info(struct broadcast_dmb_ch_info *ch_info)
{
	return ERROR;
}

/* ioctl */
int	broadcast_mtv_drv_if_get_dmb_data(struct broadcast_dmb_data_info *pdmb_data)
{
	int copy_size;
	int rc = OK;
	...
	if (mtv222_cb->tsout_enabled == true) {
		copy_size = mtv222_tsb_read((void*)((unsigned long)pdmb_data->data_buf_addr), pdmb_data->data_buf_size);
		if (copy_size > 0) {
			pdmb_data->copied_size = copy_size;
			pdmb_data->packet_cnt = copy_size / 188;

#if 0//def _MTV_KERNEL_FILE_DUMP_ENABLE
			mtv_ts_dump_kfile_write((char*)((unsigned long)pdmb_data->data_buf_addr), copy_size);
#endif
...
int	broadcast_mtv_drv_if_reset_ch(void)
{
	int rc = OK;
...
int	broadcast_mtv_drv_if_user_stop(int mode)
{
...
int	broadcast_mtv_drv_if_select_antenna(unsigned int sel)
{
...
/* file read */
int	broadcast_mtv_drv_if_read_control(char *buf, unsigned int size)
{
	int ret;
...
int	broadcast_mtv_drv_if_get_mode (unsigned short *mode)
{
