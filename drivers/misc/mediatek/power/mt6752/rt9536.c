...
struct rt9536_platform_data pdata_default = {
	.en_set = GPIO_SWCHARGER_EN_PIN ,		/* gpio for en/set */
...
static int rt9536_gpio_init(struct rt9536_info *info)
{
	...
	mt_set_gpio_mode(pdata->chgsb, GPIO_EINT_CHG_STAT_PIN_M_GPIO);
	mt_set_gpio_dir(pdata->chgsb, GPIO35_DIR);
	mt_set_gpio_pull_enable(pdata->chgsb, GPIO35_PULLEN);

	mt_set_gpio_mode(pdata->en_set, GPIO_SWCHARGER_EN_PIN_M_GPIO);
	mt_set_gpio_dir(pdata->en_set, GPIO32_DIR);
...
struct platform_device rt9536_device = {
       .name   = "rt9536",
       .id     = -1,
};
...
static int __init rt9536_init(void)
{
	...
	ret = platform_device_register(&rt9536_device);
        if (ret) {
                battery_xlog_printk(BAT_LOG_CRTI,"****[rt9536_init] Unable to device register(%d)\n", ret);
                return ret;
	}
        ret = platform_driver_register(&rt9536_driver);
	if (ret) {
                 battery_xlog_printk(BAT_LOG_CRTI,"****[rt9536_init] Unable to register driver (%d)\n", ret);
                 return ret;
        }
