			     ...
			     I32S * pmailboxok)
{
	...
	do {
		if (!(_handle->sysEnValue & TC3XREG_SYS_EN_DSP)) {
			/* Exceptional Error */
			TcpalPrintErr((I08S *)
				      "[dtv][tcc3536][error][TCC353X] MailBox - OP Disabled!!! \n");
			return;
		}
		...
		if (TcpalGetTimeIntervalCount_ms(CurrTime) >
			...
			TcpalPrintErr((I08S *)
				      "[dtv][tcc3536][error][TCC353X] MailBox Write Timeout [Command=0x%x] [OP=0x%02x%02x%02x]!!! \n",
			     ...
			     mailbox_t * p_mailbox, I32S * pmailboxok)
{
	...
	do {
		...
		if (TcpalGetTimeIntervalCount_ms(CurrTime) >
			...
			Tcc353xGetRegOpDebug(_handle, op_debug, _LOCK_);
			TcpalPrintErr((I08S *)
				      "[dtv][tcc3536][error][TCC353X] MailBox Read Timeout[OP:0x%02x%02x%02x]!!! \n",
	...
	if(total_word_num>8) {
		TcpalPrintErr((I08S *)
			      "[dtv][tcc3536][error][TCC353X] Mailbox Size error! total_byte_num=%d limit size!\n",
			      total_byte_num);
		total_byte_num = 32;
		total_word_num = 8;
	}
	...
	if ((cmd >> 24) != MB_SLAVEMAIL) {
		...
		TcpalPrintErr((I08S *)
			      "[dtv][tcc3536][error][TCC353X] Mailbox Error Cmd[0x%x] Total Byte Num[0x%x]\n",
			      cmd, total_byte_num);

		for (i = 0; i < total_word_num - 1; i++) {
			TcpalPrintErr((I08S *)
				      "[dtv][tcc3536][error][TCC353X] data_array%d=%d\n",
				      i, p_mailbox->data_array[i]);
		}

		mailstat = Tcc353xMailboxStatus(cmd, 0);
		TcpalPrintErr((I08S *)
			      "[dtv][tcc3536][error][TCC353X] Mark Error[0x%x][ori:0x%x]\n",
			      cmd, mailstat);
		TcpalPrintErr((I08S *)
			      "[dtv][tcc3536][error][TCC353X] please check the header.\n");
		pmailboxok[0] = -1;
		return TCC353X_RETURN_FAIL;
	}
	...
	if (p_mailbox->status != 0) {
		TcpalPrintErr((I08S *)
			      "[dtv][tcc3536][error][TCC353X] Error Message : 0x%0x[cmd 0x%x][totalnum 0x%x]\n",
				    ...
				    I32U * data_array, I32S word_cnt)
{
	...
	if (mailboxok == -1) {
		TcpalPrintErr((I08S *)
			      "[dtv][tcc3536][error][TCC353X] MailboxTX Error.\n");
		return TCC353X_RETURN_FAIL;
	}
			  ...
			  I32U * data_array, I32S word_cnt)
{
	...
	if (_handle->mailboxErrorCounter > MAX_MAILBOX_RETRY) {
		TcpalPrintErr((I08S *)
			      "[dtv][tcc3536][error][TCC353X][M] Critical Mailbox Control Error!!! Can't recover!!!\n");
	...
	if (ret == TCC353X_RETURN_FAIL) {
		TcpalPrintErr((I08S *)
				 "[dtv][tcc3536][error][TCC353X] MailboxTX [cmd=0x%x]\n",
				 cmd);

		/* one more time set and give up */
		TcpalPrintErr((I08S *) "[dtv][tcc3536][error][TCC353X][M] Mailbox Retry\n");
		ret =
		    Tcc353xMailboxTxOnlySub(_handle, cmd, data_array,
					    word_cnt);
		if (ret == TCC353X_RETURN_FAIL) {
			TcpalPrintErr((I08S *)
				      "[dtv][tcc3536][error][TCC353X] Can't recover mailbox. Please Retune!\n");
			_handle->mailboxErrorCounter = 0;
			TcpalSemaphoreUnLock(&Tcc353xOpMailboxSema
					     [_handle->moduleIndex]
					     [_handle->diversityIndex]);
				  ...
				  I32U * data_array, I32S word_cnt)
{
	...
	if (mailboxok == -1) {
		TcpalSemaphoreUnLock(&Tcc353xMailboxSema
				     ...
				     [_handle->diversityIndex]);
		TcpalPrintErr((I08S *)
			      "[dtv][tcc3536][error][TCC353X] MailboxTXRX Error. cmd=[%x]\n", cmd);
		return TCC353X_RETURN_FAIL;
	} else {
			...
			I32U cmd, I32U * data_array, I32S word_cnt)
{
	...
	if (ret == TCC353X_RETURN_FAIL) {
		TcpalPrintErr((I08S *)
				 "[dtv][tcc3536][error][TCC353X] MailboxTXRX [cmd=0x%x]\n",
				 cmd);
...
I32U Tcc353xGetAccessMail(Tcc353xHandle_t * _handle)
{
	...
	if (access_mail != 0x1ACCE551)
		TcpalPrintLog((I08S *)
			      "[dtv][tcc3536][error][TCC353X] AccessMail Error=0x%08x\n",
			      access_mail);
	return access_mail;
}
