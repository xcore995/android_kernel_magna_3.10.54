...
I32S Tcc353xTccspiClose(I32S _moduleIndex)
{
	...
	if (gTccSpiHanleInit0 == 0 && gTccSpiHanleInit1 == 0) {
		...
		TcpalPrintStatus((I08S *)"[dtv][tcc3536][debug_info] spi_dev :0x%p\n", spiData->spi_dev);
	}
	return TCC353X_RETURN_SUCCESS;
}
		  ...
		  I32S _size, I08U _reservedOption)
{
	...
	/* 
	At MT6582, if DMA size over 1024,DMA support 1024*n (max 1024*256)
	           else DMA suppor n bytes
	*/
	int ret = 0;
	I32S remain=0;
	I32S multi_1024=0;
	...
	if(!_bufferIn && !_bufferOut) return TCC353X_RETURN_FAIL;
        ...
        multi_1024 = ((_size>>10)<<10);
        remain = _size - multi_1024;

	//--------- Bug Fix 2014 08 05 ------------
	if(multi_1024) {
	        xfer.tx_buf = _bufferIn;
	        xfer.rx_buf = _bufferOut;
	        xfer.len = multi_1024;
    ...
    if(remain) {
        xfer.tx_buf = _bufferIn+multi_1024;
        xfer.rx_buf = _bufferOut+multi_1024;
        xfer.len = remain;
